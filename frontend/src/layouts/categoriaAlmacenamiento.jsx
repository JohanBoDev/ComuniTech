...
const CategoriaAlmacenamiento = () => {
  const { products: almacenamientos, loading, error } = useFetchProductsByCategory("almacenamiento");
  const [messages, setMessages] = useState({});
  const [marcaFilter, setMarcaFilter] = useState("");
  const [priceRange, setPriceRange] = useState([50000, 10000000]);
  const [searchTerm, setSearchTerm] = useState("");

  const filteredAlmacenamientos = almacenamientos.filter((product) => {
  ...
  return (
    <div className="py-12 bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200 dark:from-black dark:via-gray-900 dark:to-black">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8 p-10">
        {filteredAlmacenamientos.length > 0 ? (
          filteredAlmacenamientos.map((product) => (
            <div
...
export default CategoriaAlmacenamiento;
