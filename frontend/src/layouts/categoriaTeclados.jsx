...
const CategoriaTeclados = () => {
  const { products: teclados, loading, error } = useFetchProductsByCategory("teclado");
  ...
  const [priceRange, setPriceRange] = useState([50000, 10000000]); // Rango de precios
  const [searchTerm, setSearchTerm] = useState(""); // Filtro por nombre

  // Función para filtrar productos
  const filteredTeclados = teclados.filter((product) => {
  ...
  return (
    <div className="py-12 bg-cover bg-center bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200 dark:from-black dark:via-black/90 dark:to-black">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8 p-10">
        {filteredTeclados.length > 0 ? (
          filteredTeclados.map((product) => (
            <div
...
export default CategoriaTeclados;